...
class NewWordDetect:
    def __init__(self, corpus_path):
        ...
        self.word_count = defaultdict(int)  #词频字典
        self.left_neighbor = defaultdict(dict)  #左邻字字典
        self.right_neighbor = defaultdict(dict) #右邻字字典
        self.load_corpus(corpus_path)  #加载语料数据
        self.calc_pmi()  #计算互信息
    ...
    def calc_entropy_by_word_count_dict(self, word_count_dict):
        ...
        entropy = sum([-(c / total) * math.log((c / total), 10) for c in word_count_dict.values()]) #p为概率，log以10为底
        return entropy
    ...
    #计算互信息(pointwise mutual information),内部稳定度。
    def calc_pmi(self):
        self.calc_total_count_by_length()
        self.pmi = {}
        for word, count in self.word_count.items():
            p_word = count / self.word_count_by_length[len(word)] #单个字数出现次数/文档长度
            p_chars = 1
            for char in word:
                p_chars *= self.word_count[char] / self.word_count_by_length[1]
            self.pmi[word] = math.log(p_word / p_chars, 10) / len(word)  #公式。
        return
    ...
    def calc_word_values(self):
        ...
        for word in self.pmi:
            ...
            self.word_values[word] = pmi * le  * re  #三值交互可min(le,re)
