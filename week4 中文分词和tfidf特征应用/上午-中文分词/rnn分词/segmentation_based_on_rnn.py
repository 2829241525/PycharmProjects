...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x, _ = self.rnn_layer(x)  #output shape:(batch_size, sen_len, hidden_size)  前后输出分别为每个字向量，每句话向量。这里需要字向量
...
class Dataset:
    ...
    def load(self):
        ...
        with open(self.corpus_path, encoding="utf8") as f:
            for line in f:
                sequence = sentence_to_sequence(line, self.vocab) #将每个字转换为数字，根据字符表转换
