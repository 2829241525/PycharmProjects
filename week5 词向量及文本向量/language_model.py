...
"""  第五章上午0：55  。论文：《A Neural Probabilistic Language》。参照公式为： y = b+Wx+Utanh(d+Hx)
基于pytorch的语言模型
与基于窗口的词向量训练本质上非常接近
只是输入输出的预期不同
不使用向量的加和平均，而是直接拼接起来
...
class LanguageModel(nn.Module):
    def __init__(self, vocab_size, max_len, embedding_size, hidden_size):
        print(vocab_size, max_len, embedding_size, hidden_size)
        ##y = b + Wx + Utanh(d + Hx)
        super(LanguageModel, self).__init__()
        #入参：词表大小，向量纬度
        self.word_vectors = nn.Embedding(vocab_size, embedding_size)
        print("word_vectors.weight.shape:", self.word_vectors.weight)
        #x=（C1，C2，C3...）
