...
def build_dataset(tokenizer, corpus, max_length, batch_size):
    dataset = []
    for i, (answer, prompt) in enumerate(corpus):
        ...
        if len(prompt_encode) > 100:
            prompt_encode = prompt_encode[:100]
...
def main(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num):
        ...
        #海尔突击更改能效数据 涉嫌套取国家补贴
        print(generate_sentence("调查发现，海尔两款平板电视出现能效数据频繁更改的情况。此外，康佳的一款液晶平板电视的能效指数也被更改。更改之后，上述几款产品都巧妙入围近期国家三部委公布的《节能产品惠民工程高效节能平板电视推广目录》第二批中。", model, tokenizer))
        print(generate_sentence("效指数也被更改。更改之后，上述几款产品都巧妙入围近期国家三部委公布的《节能产品惠民工程高效节能平板电视推广目录》第二批中。敞篷车，铁皮桶，泔水四溢。夜幕里，回收餐厨垃圾的泔水车，穿梭于大小饭店。这些泔水车多是从大小饭店驶入北京郊区或北京周边的养猪场，未经无害化处理直接饲养生猪……餐厨垃圾企业负责人称未见过相关监管人员身影。（新京报） ", model, tokenizer))
        print(generate_sentence("研究生在网络安全或相关领域的学习中，需要掌握现代入侵检测系统（IDS）的原理、挑战与技术融合。了解IDS的工作机制，包括数据收集、流量分析和事件检测的方法。掌握各类检测方法（基于签名和基于异常）及其适用场景。", model, tokenizer))
    if not save_weight:
        return
    else:
