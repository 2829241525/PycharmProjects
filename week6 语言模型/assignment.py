...
model = BertModel.from_pretrained(r"C:\Users\Lenovo\PycharmProjects\pythonProject\week6 语言模型\bert-base-chinese", return_dict=False)
...
# self_attention_out参数 其中 embedding_size * embedding_size + embedding_size 是self输出的线性层参数，
# embedding_size + embedding_size是layer_norm层参数，残差后归一化
...
# embedding_size + embedding_size是layer_norm层，残差后归一化
feed_forward_parameters = (embedding_size * hide_size + hide_size + embedding_size * hide_size + embedding_size
                           + embedding_size + embedding_size)
